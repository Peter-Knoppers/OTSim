...
/*	    							if (outLink == ceilShareOfLanes.length)
    ...
    private Link createJunctionLink(DirectionalLink incoming, ArrayList<Lane> newLanes, ArrayList<RoadMarkerAlong> rmaList)  { 
		...
		for (int i = 1; ; i++)
			if (null == parentNetwork.lookupLink(linkName + i)) {
				linkName += i;
				break;
			}
		Link newLink = parentNetwork.addLink(linkName, expandNode1.nodeID, expandNode2.nodeID, 0, false, csList,  vertexInner);
    ...
    private Node expandNode(boolean in, Link link, CrossSectionElement inCse) {
    	...
    	if ((in && (link.getToNodeExpand() == link.getToNode_r())) || 
			...
			expandedNode = new Node(parentNetwork, nodeName, parentNetwork.nextNodeID(), v.x, v.y, v.z, true);
			parentNetwork.addExpandedNode(expandedNode);
	...
	public void paint(GraphicsPanel graphicsPanel) {
        ...
        final Color color = parentNetwork.isExpandedNode(this) ? Color.blue : Color.RED;
        graphicsPanel.setColor(color);
        if ((! parentNetwork.isExpandedNode(this)) || (Main.mainFrame.showLabelsOnAutogeneratedNodes.isSelected()))
