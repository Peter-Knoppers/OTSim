				...
				for (CrossSection cs : link.getCrossSections_r()) {
					...
					for (CrossSectionElement cse : cs.getCrossSectionElementList_r()) {
						...
						if (cse.getCrossSectionElementTypology().getDrivable()) {
							ArrayList<Vertex> linkPointList = cse.getLinkPointListInner(true, true);
							...
							linkPointList = cse.getLinkPointListOuter(true, true);
	...
	public void rebuild() {
		...
		if (dirty) {
			...
			try {
				...
				fixNodeAreas();
				// connect all successive cross sections by cs-element (weglaten???)
				...
				fixLinkConnections();
				// connect the connections between cross sections at a link
    ...
    private void fixCSEVertices()  {
    	for (Link link : getLinkList())   {
    		if (! link.isAutoGenerated())  {
	    		for (CrossSection cs : link.getCrossSections_r())  {
	    			for (CrossSectionElement cse : cs.getCrossSectionElementList_r()) {
	    				cse.setVerticesInner(cse.getLinkPointListInner(false, true));
	    				cse.setVerticesOuter(cse.getLinkPointListOuter(false, true));
	    				for (CrossSectionObject cso : Reversed.reversed(cse.getCrossSectionObjects(RoadMarkerAlong.class)))  {
	              		...
	              		cse.createLaneVertices(cse.getCrossSectionObjects(RoadMarkerAlong.class), cse.getCrossSectionObjects(Lane.class));
	    			}
	    		}
    		}
    	}
				...
				for (CrossSectionElement cse : cs.getCrossSectionElementList_r())
					for (CrossSectionObject tlcso : cse.getCrossSectionObjects(TrafficLight.class)) {
						...
						for (CrossSectionObject lcso : cse.getCrossSectionObjects(Lane.class)) {
							...
							if (position < 0)
								position += Planar.length(tl.crossSectionElement.getLinkPointList(CrossSectionElement.LateralReferenceCenter, true));
