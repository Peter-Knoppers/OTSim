...
/**
 * Editable polygon that may not be self-intersecting.
 * Vertices are actually 3D, the X and Y components must define a non-self-intersecting polygon
 * 
...
public class SimplePolygon implements XML_IO {
	...
	/**
	 * Create a SimplePolygon with no name and vertices derived from an ordered
	 * list of Point2D.Doube and a common z value.
	...
	/**
	 * Create a SimplePolygon with no name and vertices from an ordered list of
	...
	private boolean writeVertices(StaXWriter staXWriter) {
		for (int i = 0; i < vertices.size(); i++) {
			...
			if (! (staXWriter.writeNodeStart(XML_VERTEX, hashMap)
					&& vertices.get(i).writeVertexXML(staXWriter)
					&& staXWriter.writeNodeEnd(XML_VERTEX)))
				return false;
		}
		return true;
	}
	...
	/**
	 * Generate an array of 2D points of the z=0 projection of this 
	 * SimplePolygon.
	...
	/**
	 * Compute the surface area of the z=0 projection of this SimplePolygon.
	 * <br /> Derived from <a href="http://geomalgorithms.com/a01-_area.html#2D%20Polygons">Areas
	 * of Triangles and Polygons</a>.
	...
	/**
	 * Check that a proposed value of a Vertex is permissible. 
	 * A vertex may be added if the result is a valid SimplePolygin (the edges 
	 * do not self-intersect and the surface area is >= 0).
	 * @param i Integer; index of the Vertex that might be changed
	 * @param v {@link Vertex}; the proposed new Vertex
	 * @return Boolean; true if the proposed new Vertex is permissible; false
	 * if the proposed new Vertex is not permissible
	 */
	...
	/**
	 * Check that a proposed vertex may be added to the end of this SimplePolygon.
	 * A vertex may be added if the result is a valid SimplePolygin (the edges 
	 * do not self-intersect and the surface area is >= 0).
	 * @param v {@link Vertex}; the proposed vertex
	 * @return Boolean; true if the proposed vertex is permissible; false if
	 * the proposed vertex is not permissible
	 */
	...
	/**
	 * Check that a vertex may be deleted from this SimplePolygon. A vertex may
	 * be deleted if the result is a valid SimplePolygin (the edges do not
	 * self-intersect and the surface area is >= 0).
	 * @param i Integer; index of the vertex to check for deletion.
	 * @return Boolean; true if the indicated vertex may be deleted; false if
	 * the indicated vertex may not be deleted
	 */
